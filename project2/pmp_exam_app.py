...
for q in questions:
    while True:
        ...
        for option in q["options"]:
            print(option)
        answer = input("Enter your choice (A, B, C, or D) or type 'exit' to quit: ").strip().upper()
        if answer == "EXIT":
            print(f"\nExiting the test. Your final score: {score}/{len(questions)}")
            exit()
...
print(f"Test completed! Your final score: {score}/{len(questions)}")
def conduct_test():
    score = 0
